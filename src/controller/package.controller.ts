...
export const packageController = new Elysia({ prefix: "/packages" })
  ...
  .get("/", async ({ jwt, headers, user, prisma }) => {
    ...
    const packages = [] as { name: string, image_url: string, id: number, quantity: number }[];
    for (const id of ids) {
    ...
    async ({ user, prisma, body }) => {
      ...
      const packages = await prisma.package.findMany({
        ...
        const packagesUser = await prisma.packages_User.findMany({
            where: { userId: user.id, packageId: { in: packagesId }, opened: false },
        });
        console.log({ packagesUser: packagesUser.length, packagesId: packagesId.length })
      ...
      for await (const package_ of packages) {
        ...
        const packageUserId = packagesUser.find(p => p.packageId === package_.id)?.id || 10000;
        if (packageUserId === 10000) {
            console.log('Package not found');
            continue;
        };
        await prisma.packages_User.update({
            where: { userId: user.id, packageId: package_.id, id: packageUserId },
            data: { opened: true }
        });
      }
