...
export async function OpenPackage(pkg: Package, prisma: PrismaClient): Promise<Card[]> {
    const cards = [] as Card[]
    for await (const card of Array.from({ length: pkg.cards_quantity}, (_, i) => i)) {
        ...
        console.log('Raridade: ', rarity, pkg[(rarity + '_rarity') as keyof Package])
        const count = await prisma.card.count({ where: { rarity: mapRarity[rarity] } })
        const random = Math.floor(Math.random() * count)
        const card_ = await prisma.card.findFirst({ where: { rarity: mapRarity[rarity] }, skip: random })
