...
async function seedUsers() {
  console.time("seed");
  console.log(
    "Seeding users and other models, this can take five minutes or more..."
  );

  // ITERATIONS control:
  const USERS = 40;
  const CARDS_PER_USER = 40;
  const TRADES_PER_USER = 20;
  ...
  const firstId = await prisma.package.findFirst({
    orderBy: { id: "asc" },
    select: { id: true },
  });
  for (const user of new_users) {
    ...
    for (let i = 0; i < PACKAGES_PER_USER; i++) {
      const _user = await prisma.packages_User.create({
        data: {
          userId: user.id,
          packageId: Math.floor(Math.random() * 7) + (firstId?.id || 1),
        },
      });
    }
  }
  ...
  for (const user of prisma_users) {
    for (let i = 0; i < TRADES_PER_USER; i++) {
      ...
      if (_user.id !== user.id) {
        const _user_cards = await prisma.cards_user.findMany({
          take: 12,
